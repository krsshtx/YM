...
void setup()
{
  ...
  //SD
  if(!SD.begin(PA4, SPI_FULL_SPEED ))
  {
 ...
 //        topUpBuffer();
  ...
  while(waitSamples == 0 )
  {
   ...
   waitstate=false; 
     ...
     //  Serial.print("tdlt ");Serial.println(tdlt);
     //  Serial.print("w ");Serial.println(waitSamples);
  ...
  tdlt=(endt-startt)/23;
        ...
        if(tdlt > 0 && waitSamples >= tdlt){
        waitSamples=waitSamples-tdlt;
        waittdlt=true;  
         if(tdlt > 1 ) { 
         // Serial.print("waitSamples ");Serial.println(waitSamples);
         // Serial.print("tdlt ");Serial.println(tdlt);
         }
        }
      
        
        if(waitSamples > 0 && waittdlt==false) 
         {
         waitSamples--;
         waitstate=true;
         }
  ...
  if (endt-startt<5 && waitstate == true ) {
//  if(ramPrefetchFlag)
//  {
//    ramPrefetch = ram.ReadByte(pcmBufferPosition);
